...
PRO iquvextract11spotchecker


  common share1, W50double, W10double, EDOTdouble
...
readcol, /silent, '/u/tor04b/IDLWorkspace83/Default/' + Jname[pulsarnumber] + '_20CM_coadded.SFTC.ascii',$
...
;for i = numbinsperperiod-1, 0, -1 do begin
  ;this program is only for spotchecking pulsars, no more bulk pulsars
for i = 563, 0, -1 do begin  
...
device, filename= Jname[pulsarnumber] + '.20CM.Profile.ps'
...
highpapsidiag = ((myW10bin2*360.)/thebinsize)-180.+.2*(((myW10bin2*360.)/thebinsize)-180.)
...
plot, longitude,Iabsolute,$
  min_value=-50.0,$
  xrange=[lowpapsi,highpapsidiag],$
  yrange=[bigmin + 0.1*bigmin,max(Iabsolute)+0.1*max(Iabsolute)],$
  PSYM = 7,$
  ;symbol style
  symsize=.3,$
  title= '' ,xtitle='Longitude,' + cgGreek('phi') + '[deg]' ,ytitle='Intensity',$
  xstyle=1,ystyle=1, _extra = gang_plot_pos(3,1,2)
background=100200400
color=0
;plot the array with bin on x and I on y.

oplot, longitude, Qshift, linestyle = 5, color=cgcolor('red'), _extra = gang_plot_pos(3,1,2)
oplot, longitude, Ushift, linestyle = 4, color=cgcolor('blue'),_extra = gang_plot_pos(3,1,2)
oplot, longitude, Vabsolute, linestyle = 3, color=cgcolor('purple'),_extra = gang_plot_pos(3,1,2)
oplot, longitude, Ltrue, linestyle = 6, color=cgcolor('orange'), _extra = gang_plot_pos(3,1,1)


xyouts, wwindow[pulsarnumber]-wwindow[pulsarnumber]*.20,max(Iabsolute)+max(Iabsolute)*.0, 'Black = I', color=cgcolor('black'), charsize=0.5, /data,_extra = gang_plot_pos(3,1,2)
xyouts, wwindow[pulsarnumber]-wwindow[pulsarnumber]*.20,max(Iabsolute)-max(Iabsolute)*.10, 'Red = Q', color=cgcolor('red'), charsize=0.5, /data,_extra = gang_plot_pos(3,1,2)
xyouts, wwindow[pulsarnumber]-wwindow[pulsarnumber]*.20,max(Iabsolute)-max(Iabsolute)*.20, 'Blue = U', color=cgcolor('blue'), charsize=0.5, /data,_extra = gang_plot_pos(3,1,2)
xyouts, wwindow[pulsarnumber]-wwindow[pulsarnumber]*.20,max(Iabsolute)-max(Iabsolute)*.30, 'Purple = V', color=cgcolor('purple'), charsize=0.5, /data,_extra = gang_plot_pos(3,1,2)
xyouts, wwindow[pulsarnumber]-wwindow[pulsarnumber]*.20,max(Iabsolute)-max(Iabsolute)*.40, 'Orange = L', color=cgcolor('orange'), charsize=0.5, /data,_extra = gang_plot_pos(3,1,2)
xyouts, wwindow[pulsarnumber]-wwindow[pulsarnumber]*1.9, max(Iabsolute), 'W10/W50 Diagnosis Plot', color=cgcolor('black'), charsize=0.5, /data,_extra = gang_plot_pos(3,1,2)
...
oplot,W50negx, W50negy, color=cgcolor('black'),thick=2,_extra = gang_plot_pos(3,1,2)
;plots the first W50 line

W50posx=[btl502,btl502]
W50posy=[-1000000000000000,40000000000000]
oplot,W50posx, W50posy, color=cgcolor('black'),thick=2, _extra = gang_plot_pos(3,1,2)
;plots the second W50 line

W10negx=[btl101,btl101]
W10negy=[-100000000000000,4000000000000000]
oplot,W10negx, W10negy, color=cgcolor('pink'),thick=2, _extra = gang_plot_pos(3,1,2)
;plots the first W10 line

W10posx=[btl102,btl102]
W10posy=[-1000000000000,40000000000000000]
oplot,W10posx, W10posy, color=cgcolor('pink'),thick=2, _extra = gang_plot_pos(3,1,2)
...
;plot procedure for the Paramater Plot


topplot=max(Iabsolute)+0.05*max(Iabsolute)
;print, max(Iabsolute)
;print, topplot
...
plot, Bindex,Iarray,$
  ...
  ;symbol style
  symsize=.3,$
  title='',xtitle='' ,ytitle='Intensity',$
   xstyle=1,ystyle=1, _extra = gang_plot_pos(3,1,1)
background=100200400
color=0
;plot the array with bin on x and I on y.

oplot, Bindex, Larray, linestyle = 6, color=cgcolor('orange'), _extra = gang_plot_pos(3,1,1)
oplot, Bindex, Qarray, linestyle = 5, color=cgcolor('red'), _extra = gang_plot_pos(3,1,1)
oplot, Bindex, Uarray, linestyle = 4, color=cgcolor('blue'), _extra = gang_plot_pos(3,1,1)
oplot, Bindex, Varray, linestyle = 3, color=cgcolor('purple'), _extra = gang_plot_pos(3,1,1)
...
plot,longitude,papsinew,$
  ...
  ;symbol style
  symsize=.3,$
  title=Jname[pulsarnumber] + ' 20CM',xtitle='' ,ytitle='P.A. (deg.)',$
  xstyle=1,ystyle=1, _extra = gang_plot_pos(3,1,3)
background=100200400
color=0
